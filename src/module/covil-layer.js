// from moulinette-core package.
export class CovilLayer extends PlaceablesLayer {
  constructor(...args) {
    super(...args);
  ...
  _onClickLeft(event) {
    ...
    if (canvas.dimensions.rect.contains(coords[0], coords[1])) {
      game.covil.forge.forEach((f) =>
        f.instance.onLeftClickGrid({
          x: coords[0],
          y: coords[1],
